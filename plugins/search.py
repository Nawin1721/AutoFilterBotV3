...
async def search_files(update, context):
    ...
    # SEARCHING MESSAGE
    # =========================
    search_msg = await msg.reply_text("🔍 Searching...")
    ...
    # =========================
    results = (
        await files_col.find(
            {
                "$or": [
                    {"search_text": {"$regex": search_pattern, "$options": "i"}},
                    {"caption": {"$regex": search_pattern, "$options": "i"}},
                ]
            }
        )
        .sort("message_id", -1)
    ...
    if not results:
        ...
        context.application.job_queue.run_once(
            delete_message,
            when=10,
            data={"chat_id": search_msg.chat_id, "message_id": search_msg.message_id},
        )
        ...
        # SUGGESTIONS
        all_files = await files_col.find({}, {"file_name": 1}).to_list(length=300)

        movie_names = [x["file_name"] for x in all_files]
    ...
    # AUTO CLEAR CACHE
    # =========================
    context.application.job_queue.run_once(clear_cache, when=600, data=search_id)
...
# HANDLER
# =========================
search_handler = MessageHandler(filters.TEXT & ~filters.COMMAND, search_files)
